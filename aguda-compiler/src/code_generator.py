...
class CodeGenerator:
    ...
    def callPrint(self, ctx: SymbolTable[Tuple[ir.Value, Type]], exp: Exp) -> Tuple[ir.Value, Type]:
        ...
        match arg_type:
            case BaseType("Int"):
                str_ptr = self.add_global_string("%d\0")
                self.builder.call(printf_func, [str_ptr, arg_val])
                
            case BaseType("Bool"):
                true_ptr = self.add_global_string("true\0")
                false_ptr = self.add_global_string("false\0")
                selected_ptr = self.builder.select(arg_val, true_ptr, false_ptr)
                self.builder.call(printf_func, [selected_ptr])
                
            case BaseType("Unit"):
                str_ptr = self.add_global_string("unit\0")
    ...
    def add_global_string(self, string: str) -> ir.Value:
        """
